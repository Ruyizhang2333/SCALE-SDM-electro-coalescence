...
pal <- c(colorRampPalette(c(rgb(0,0,0.1,0.1), rgb(0,0,1,0.9)), alpha = TRUE),
         colorRampPalette(c(rgb(0,0.1,0,0.1), rgb(0,1,0,0.9)), alpha = TRUE),
         colorRampPalette(c(rgb(0.1,0,0,0.1), rgb(1,0,0,0.9)), alpha = TRUE))
...
#for(time in alltimes[which(as.numeric(alltimes)==2400)]){
    ...
    dev_aspect <- dev.cur()
    image(equr.bin, polr.bin, diag(nbins)*0.0e0, asp=1,
		   zlim=c(4.0,7.0),
    		   col = rgb(1,1,1,0),
    ...
    dev_density <- dev.cur()
    image(maxdim.bin, dens.bin, diag(nbins)*0.0e0,
		   zlim=c(4.0,7.0),
    		   col = rgb(1,1,1,0),
    ...
    dev_mass <- dev.cur()
    image(maxdim.bin, mass.bin, diag(nbins)*0.0e0,
    ...
    dev_vel <- dev.cur()
    image(maxdim.bin, vel.bin, diag(nbins)*0.0e0,
		   zlim=c(4.0,7.0), #nlevel=100,
    		   col = rgb(1,1,1,0),
...
#    for(ice_category in c("snow")){
	...
	dev.set(dev_density)
	par(new=T)
	image(maxdim.bin, dens.bin, pmin(
	...
	dev.set(dev_mass)
	par(new=T)
	image(maxdim.bin, mass.bin, pmin(
