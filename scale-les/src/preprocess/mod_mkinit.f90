...
!    call rng_save_state( rng_s2c_i, basename, fid_output )
  ...
  subroutine read_sounding( &
    ...
    integer, parameter :: EXP_klim = 1000
    integer            :: EXP_kmax
...
!      RHOT(k,i,j) = RHO(k) * POTT(k)
  ...
  subroutine MKINIT_sonde_perturb
    ...
    if( IO_LNML ) write(IO_FID_LOG,nml=PARAM_MKINIT_SONDE_PERTURB)

    !!!!!! read sonde data
    call read_sounding( RHO1D, VELX1D, VELY1D, POTT1D, QV1D ) ! (out)
    ...
    do k = KS, KE
       if( GRID_CZ(k) <= RANDOM_LIMIT_Z ) then
          RHOT(k,i,j) = RHOT(k,i,j) + (2.0_RP*(rndm(k,i,j)-0.50_RP))*PERTURB_AMP*DENS(k,i,j)
       endif
    enddo
    enddo
    enddo
